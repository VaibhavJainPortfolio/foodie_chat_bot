...
user_key = ""
config = {"user_key": "c0c4de4f7de2d477c51c6100bf0eaa59"}
...
class ActionSearchRestaurants(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        print("action_restaurant",budget,location,cuisine)
                    ...
                    if location.lower() == city_name.lower():
                        ...
                        if len(restaurants_found) > 0:
                            ...
                            for index in range(0, number_of_records):
                                ...
                                if index < 5:
                                    print("Index*****",index,restaurant["name"])
                                    response_message = (
                                        response_message
                                        + "\n"
                                        + str(index + 1)
...
class ActionValidateLocation(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if not location:
            ...
            filepath = DEFAULT_DATA_PATH + "/cities.txt"
            print("File Path",filepath)
            with open(filepath) as cities_file:
                cities = [line.strip().lower() for line in cities_file]

            location_validity = ("invalid" if location.lower() not in cities else "valid")
            print("location_validity",location_validity)
